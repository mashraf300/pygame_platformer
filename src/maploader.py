...
class MapLoader():
    ...
    def load(self):
        ...
        path = os.path.join("Maps","map1","level.map")
        with open(path) as f:
            map_txt =  f.read()
    ...
    def createLayers(self):
        ...
        imgs = os.listdir(os.path.join("Maps","map1","layers"))
        for index in range(1,len(imgs)):
            image = pygame.image.load(os.path.join("Maps","map1","layers", "%i.png"%(index))).convert_alpha()
